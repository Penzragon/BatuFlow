...
export default function DeliveryOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/delivery-orders?pageSize=100");
  ...
  const openCreateFromSO = async (soId: string) => {
    try {
      ...
      if (so.deliveryOrders) {
        for (const dOrder of so.deliveryOrders) {
          if (dOrder.status === "CONFIRMED" || dOrder.status === "DRAFT") {
            ...
            if (doJson.success) {
              for (const line of doJson.data.lines) {
                deliveredQtyMap.set(
                  line.productId,
      ...
      if (linesWithRemaining.length === 0) {
        toast.error("All items have already been delivered");
        return;
      }

      setSOLines(linesWithRemaining);
      setCreateOpen(true);
    } catch {
  ...
  const handleCreateDO = async () => {
    ...
    try {
      const payload = {
        salesOrderId: createFromSO,
        notes: createNotes || undefined,
        lines: soLines
          .filter((l) => l.qtyToDeliver > 0)
          .map((l) => ({
            productId: l.productId,
            qtyDelivered: l.qtyToDeliver,
          })),
      };
      ...
      if (json.success) {
        ...
        setCreateOpen(false);
        router.push(`/sales/delivery-orders/${json.data.id}`);
      } else {
        toast.error(json.error || "Failed to create delivery order");
      }
    } catch {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <button
            ...
            onClick={() => router.push(`/sales/delivery-orders/${row.original.id}`)}
          >
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <button
            ...
            onClick={() => router.push(`/sales/orders/${row.original.salesOrder.id}`)}
          >
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-lg">
            ...
            {soLines.map((line, idx) => (
              <div key={line.productId} className="flex items-center gap-3 text-sm">
                <div className="flex-1">
                  ...
                  <span className="text-muted-foreground ml-1">({line.productSku})</span>
                  <div className="text-xs text-muted-foreground">
                    {t("remainingQty")}: {line.remaining} {line.uom}
                  </div>
                </div>
                <div className="w-24">
                  <Input
                    ...
                    onChange={(e) => {
                      const val = Math.min(parseFloat(e.target.value) || 0, line.remaining);
                      setSOLines((prev) => {
                        const updated = [...prev];
                        updated[idx] = { ...updated[idx], qtyToDeliver: val };
                        return updated;
                      });
                    }}
                  />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateDO} disabled={submitting}>
              {submitting ? tc("loading") : t("createDO")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
