...
export default function ReceiptCategoriesPage() {
  const t = useTranslations("receiptCategories");
  ...
  const [editing, setEditing] = useState<ReceiptCategory | null>(null);
  const [formData, setFormData] = useState({ name: "", coaAccountId: "" });
  const [saving, setSaving] = useState(false);

  const [deleteTarget, setDeleteTarget] = useState<ReceiptCategory | null>(
    null
  );
  ...
  function openEditDialog(category: ReceiptCategory) {
    setEditing(category);
    setFormData({
      name: category.name,
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = editing
        ? `/api/receipt-categories/${editing.id}`
        : "/api/receipt-categories";
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/receipt-categories/${deleteTarget.id}`, {
        method: "DELETE",
      });
      const json = await res.json();
      if (json.success) {
  ...
  const columns: ColumnDef<ReceiptCategory>[] = useMemo(
    () => [
      {
        accessorKey: "name",
        header: t("name"),
