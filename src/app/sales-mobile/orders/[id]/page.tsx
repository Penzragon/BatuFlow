    ...
    <div className="space-y-4 p-4 pb-8">
      ...
      <div className="rounded-lg border p-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${statusClass[so.status] ?? "bg-slate-100 text-slate-700"}`}>
            {ts(so.status)}
          </span>
        </div>
      ...
      <div className="rounded-lg border p-3">
        ...
        <div className="space-y-2">
          {so.lines.map((line) => (
            <div key={line.id} className="rounded-md border p-2 text-xs">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="font-medium">{line.product.name}</p>
                  <p className="text-muted-foreground">{line.product.sku}</p>
                </div>
                <p className="font-medium">{formatCurrency(Number(line.lineTotal))}</p>
              </div>
              <div className="mt-2 grid grid-cols-2 gap-2 text-muted-foreground">
                <p>{t("qty")}: <span className="font-medium text-foreground">{line.qty} {line.uom}</span></p>
                <p>{t("unitPrice")}: <span className="font-medium text-foreground">{formatCurrency(Number(line.unitPrice))}</span></p>
                <p>{t("discount")}: <span className="font-medium text-foreground">{Number(line.discountPercent).toFixed(2)}%</span></p>
                <p>{t("discountAmount")}: <span className="font-medium text-foreground">{formatCurrency(Number(line.discountAmount))}</span></p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-lg border p-3 text-sm">
        <p className="mb-2 font-semibold">{t("summaryTitle")}</p>
